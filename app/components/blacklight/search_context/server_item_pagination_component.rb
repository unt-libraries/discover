...
module Blacklight
  module SearchContext
    class ServerItemPaginationComponent < Blacklight::Component
      ...
      # Overrides Blacklight to use different translate key and add inactive class
      def link_to_previous_document(previous_document = nil, classes: 'previous', **link_opts)
        ...
        link_to_unless previous_document.nil?, raw(t('blacklight.search.entry_pagination_info.previous')), url_for_document(previous_document), link_opts do
          tag.span raw(t('blacklight.search.entry_pagination_info.previous')), class: 'previous inactive'
        end
      end
      ...
      def link_to_next_document(next_document = nil, classes: 'next', **link_opts)
        ...
        link_to_unless next_document.nil?, raw(t('blacklight.search.entry_pagination_info.next')), url_for_document(next_document), link_opts do
          tag.span raw(t('blacklight.search.entry_pagination_info.next')), class: 'next inactive'
        end
      end

      def html_classes
        load_yaml_data('www/html_classes')['breadcrumbs']
      end
