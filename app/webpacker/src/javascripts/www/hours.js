...
export class LibraryHours {
    ...
    async init() {
      let storedHours = getWithExpiry(this.config.storageKey, true); // Attempt to get stored hours from session storage
      if (!storedHours) {
    ...
    async fetchHours() {
        ...
        //const hoursData = await fetchData(`${url}&to=${nextWeek}`, 30000); // 30-second timeout
        const hoursData = await fetchData(`${url}?to=${nextWeek}`, 30000); // 30-second timeout // with proxy!
        ...
        setWithExpiry(this.config.storageKey, storeData, this.config.expires, true);
