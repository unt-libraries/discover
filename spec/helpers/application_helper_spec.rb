...
describe ApplicationHelper do
  ...
  describe '#get_number_search_results' do
    ...
    context 'when search results do not have a total count' do
      it 'returns 0 with delimiter' do
        ...
        expect(helper.get_number_search_results(response)).to eq nil
      end
    end
  ...
  describe '#bootstrap_select_tag' do
    context 'when multiple option is not provided' do
      it 'renders select tag without multiple attribute' do
        result = helper.bootstrap_select_tag('example', '<option value="1">Option 1</option>'.html_safe)
    ...
    context 'when multiple option is provided' do
      it 'renders select tag with multiple attribute' do
        result = helper.bootstrap_select_tag('example', '<option value="1">Option 1</option>'.html_safe, multiple: true)
        parsed_result = Nokogiri::HTML.fragment(result)
    ...
    context 'when include_blank option is provided' do
      it 'renders select tag with a blank option' do
        result = helper.bootstrap_select_tag('example', '<option value="1">Option 1</option>'.html_safe, include_blank: true)
        parsed_result = Nokogiri::HTML.fragment(result)

        expect(parsed_result.at_css('div')['name']).to eq 'example'
        expect(parsed_result.at_css('div')['id']).to eq 'example'
        expect(parsed_result.at_css('div a')['value']).to eq ''
        expect(parsed_result.at_css('div option')['value']).to eq '1'
        expect(parsed_result.at_css('div option').text).to eq 'Option 1'
      end
    end

    context 'when prompt option is provided' do
      it 'renders select tag with a prompt option' do
        result = helper.bootstrap_select_tag('example', '<option value="1">Option 1</option>'.html_safe, prompt: 'Select an option')
        parsed_result = Nokogiri::HTML.fragment(result)

        expect(parsed_result.at_css('div')['name']).to eq 'example'
        expect(parsed_result.at_css('div')['id']).to eq 'example'
        expect(parsed_result.at_css('div a')['value']).to eq ''
        expect(parsed_result.at_css('div a').text).to eq 'Select an option'
        expect(parsed_result.at_css('div option')['value']).to eq '1'
        expect(parsed_result.at_css('div option').text).to eq 'Option 1'
      end
    end

    context 'when both include_blank and prompt options are provided' do
      it 'renders select tag with both blank and prompt options' do
        result = helper.bootstrap_select_tag('example', '<option value="1">Option 1</option>'.html_safe, include_blank: true, prompt: 'Select an option')
        parsed_result = Nokogiri::HTML.fragment(result)

        expect(parsed_result.at_css('div')['name']).to eq 'example'
        expect(parsed_result.at_css('div')['id']).to eq 'example'
        expect(parsed_result.css('div a').length).to eq 2
        expect(parsed_result.css('div a')[0]['value']).to eq ''
        expect(parsed_result.css('div a')[0].text).to eq 'Select an option'
        expect(parsed_result.css('div a')[1]['value']).to eq ''
        expect(parsed_result.css('div a')[1].text).to eq ''
        expect(parsed_result.at_css('div option')['value']).to eq '1'
        expect(parsed_result.at_css('div option').text).to eq 'Option 1'
      end
    end

    context 'when custom include_blank option is provided' do
      it 'renders select tag with custom blank option' do
        result = helper.bootstrap_select_tag('example', '<option value="1">Option 1</option>'.html_safe, include_blank: 'Custom Blank')
        parsed_result = Nokogiri::HTML.fragment(result)

        expect(parsed_result.at_css('div')['name']).to eq 'example'
        expect(parsed_result.at_css('div')['id']).to eq 'example'
        expect(parsed_result.at_css('div a')['value']).to eq ''
        expect(parsed_result.at_css('div a').text).to eq 'Custom Blank'
        expect(parsed_result.at_css('div option')['value']).to eq '1'
        expect(parsed_result.at_css('div option').text).to eq 'Option 1'
      end
    end
  ...
  describe '#bootstrap_options_for_select' do
    let(:options) { [['Option 1', '1'], ['Option 2', '2']] }

    it 'renders options with correct attributes' do
      result = helper.bootstrap_options_for_select(options)
      parsed_result = Nokogiri::HTML.fragment(result)

      expect(parsed_result.css('a').length).to eq 2
      expect(parsed_result.css('a')[0]['class']).to eq 'dropdown-item'
      expect(parsed_result.css('a')[0]['data-search-field']).to eq '1'
      expect(parsed_result.css('a')[0]['data-pretty']).to eq 'Option 1'
      expect(parsed_result.css('a')[0].text).to eq 'Option 1'

      expect(parsed_result.css('a')[1]['class']).to eq 'dropdown-item'
      expect(parsed_result.css('a')[1]['data-search-field']).to eq '2'
      expect(parsed_result.css('a')[1]['data-pretty']).to eq 'Option 2'
      expect(parsed_result.css('a')[1].text).to eq 'Option 2'
    end

    context 'when selected value is provided' do
      it 'marks the selected value as selected' do
        result = helper.bootstrap_options_for_select(options, '1')
        parsed_result = Nokogiri::HTML.fragment(result)

        expect(parsed_result.css('a')[0]['selected']).to eq 'selected'
        expect(parsed_result.css('a')[1]['selected']).to be_nil
      end
    end

    context 'when disabled value is provided' do
      it 'marks the disabled value as disabled' do
        result = helper.bootstrap_options_for_select(options, disabled: '2')
        parsed_result = Nokogiri::HTML.fragment(result)

        expect(parsed_result.css('a')[0]['disabled']).to be_nil
        expect(parsed_result.css('a')[1]['disabled']).to eq 'disabled'
      end
    end
  end
